...
<nav>
	...
	<form action="">
		<input type="text" name="Barcode" placeholder="Input barcode here"/ >
		<button type="submit">Submit</button>
		<br/>

	</form>
		<p>All network copies matching <span style="font-weight:bold"><?php echo $_GET['Barcode']; ?></span></p>
		<p>Title -- <span style="font-weight:bold">Barcode</span> -- Process Type (Lost, Missing, etc) -- <span style="color:red">COMMITTED TO RETAIN?</span> -- Internal Note 2 (Old Retention Note)</p>
			<?php
				foreach ($networkid_xml->QueryResult->ResultXml->rowset->Row as $item ) {printf('<li>%s -- <span style="font-weight:bold">%s</span> -- %s -- <span style="color:red">%s</span> -- %s</li>', $item->Column2, $item->Column4, $item->Column7, $item->Column5, $item->Column6);}
			?>
		</div></div></div></div>
</body>
</html>
