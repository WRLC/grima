      ...
      <div class="container task-AdminAddInstitution">
        ...
        <div class="card">
		  ...
		  <div class="card-body">
			...
			<form action="">
				<input type="text" name="Barcode" autofocus="autofocus" placeholder="insert barcode here"/>
				<button type="submit">Submit</button>
			<br/>
			</form>
		<p>All volumes of title matching <span style="font-weight:bold"><?php echo $_GET['Barcode']; ?></span></p>
		<table border= '2'>
        <tr>
          <th>ISSN</th>
          <th>Title</th>
          ...
          <th><span style="font-weight:bold">Volume</span></th>
    			...
    			<?php
				foreach ($ISSN_xml->QueryResult->ResultXml->rowset->Row as $item ) {printf('<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td><span style="font-weight:bold">%s</span></td></tr>', $item->Column1, $item->Column2, $item->Column4, $item->Column7, $item->Column6);}      			
          ?>
    </table>
	</div></div></div></div>
</body>
